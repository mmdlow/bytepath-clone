...
function Explosion:new(area, x, y, opts)
  ...
  self.depth = 75
  self.damage = 200
  local w = (opts.w2 or random(48, 56)) * current_room.player.area_multiplier

  self.projectiles_explosions = current_room.player.projectiles_explosions
  ...
  if self.projectiles_explosions then
    ...
    for i = 1, 5 + current_room.player.additional_barrage_projectiles do
      ...
      local d = 2.2 * current_room.player.w
      local r = current_room.player.r
      self.area:addGameObject('Projectile',
        self.x + d * math.cos(r + random_angle),
        self.y + d * math.sin(r + random_angle),
        {r = r + random_angle, attack = 'Neutral'})
    end
    self.dead = true
...
function Explosion:draw()
  if self.projectiles_explosions then return end
